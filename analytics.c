...
void aMatrix(int index, int attempt) {
  ...
  for (int i = 0; i < quizlist.quiz[index].no_of_questions; ++i) {
    ...
    if (i+1<10) {
      printf(" %d) [%c] (%d/%d)   ", i+1, response[index][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][0]][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][1]][currentuser.ID].status, userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].marks[i] ,question[index][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][0]][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][1]].marks);
    }
    else {
      printf("%d) [%c] (%d/%d)   ", i+1, response[index][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][0]][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][1]][currentuser.ID].status, userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].marks[i], question[index][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][0]][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][1]].marks);
    }
...
void analyseQuestion(int i, int index, int attempt) {
  ...
  printf("Your answer:\n%s\n\n", response[index][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][0]][userlist[currentuser.ID].quizes_attempted[index].attempt[attempt].q_bank[i][1]][currentuser.ID].answer);
