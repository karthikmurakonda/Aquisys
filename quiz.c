...
void qMatrix(int index, int attempt) {
  ...
  for (int i = 0; i < quizlist.quiz[index].no_of_questions; ++i) {
    ...
    if (i+1<10) {
      printf(" %d) [%c] (%d)   ", i+1, quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].status, quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].marks);
    }
    else {
      printf("%d) [%c] (%d)   ", i+1, quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].status, quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].marks);
    }
...
void askQuestion(int i, int index, int attempt) {
  printf("-------------------------------------------\n                  %s               \n", quizlist.quiz[index].name);
  printf("-------------------------------------------\n             Question %d (%d Marks)\n\n", i+1, quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].marks);
  printf("Question:\n");
  ...
  if (quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].status=='A') {
    printf("Your current answer:\n%s\n\n", quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].answer);
      printf("Change the answer (enter # to exit without changing answer):\n");
  }
  else {
      printf("Enter the answer (enter # to exit without answering):\n");
      //Set status to seen
      quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].status='S';
  }
  ...
  if (answer[0]!='#') {
    //Update response
    strcpy(quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].answer, answer);
    //Mark as attempted
    quizlist.quiz[index].question[currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][0]][currentuser.quizes_attempted[index].attempt[attempt].q_bank[i][1]].response[currentuser.ID].status='A';
  }
  clearscr();
